/**
 * Generic Tab Manager Hook
 *
 * Reusable state management for tab systems.
 * Works with any data type via TypeScript generics.
 * No UI dependencies - pure state management.
 */

import { useState, useEffect, useCallback, useMemo } from 'react';
import type { TabItem, TabsState, UseTabManagerOptions, TabManagerReturn } from './types';

/**
 * Custom hook for managing tab state
 *
 * @template T - The type of tab identifiers (usually string)
 * @param options - Configuration options
 * @returns Tab management functions and state
 *
 * @example
 * ```typescript
 * const tabs = useTabManager<string>({
 ...
 * ```
 */
export function useTabManager<T = string>(
  ...
  const [state, setState] = useState<TabsState<T>>(() => {
    if (persist) {
      const stored = localStorage.getItem(storageKey);
      if (stored) {
        try {
          const parsed = JSON.parse(stored) as TabsState<T>;
          return parsed;
        } catch (error) {
