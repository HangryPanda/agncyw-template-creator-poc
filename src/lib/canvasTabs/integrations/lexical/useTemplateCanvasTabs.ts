 ...
 *
 * Template-specific wrapper around the generic useCanvasTabsManager hook.
...
import { useCanvasTabsManager } from '../../core';

export interface UseTemplateCanvasTabsReturn {
  /**
 ...
 */
export function useTemplateCanvasTabs(): UseTemplateCanvasTabsReturn {
  const [dirtyTabs, setDirtyTabs] = useState<Set<string>>(new Set());

  const tabManager = useCanvasTabsManager<string>({
    storageKey: 'insurance_template_canvas_tabs',
    persist: true,
    maxTabs: 15,
