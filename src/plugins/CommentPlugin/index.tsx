...
function ShowDeleteCommentOrThreadDialog({
  ...
  return (
    <>
      Are you sure you want to delete this {'type' in commentOrThread ? commentOrThread.type : 'comment'}?
...
function CommentsPanelList({
  ...
  comments: Array<CommentOrThread>;
  deleteCommentOrThread: (
    commentOrThread: Comment | Thread,
    thread?: Thread,
  ) => void;
  ...
  return (
    <ul className="CommentPlugin_CommentsPanel_List" ref={listRef}>
      {comments.map((commentOrThread: CommentOrThread) => {
        const id = commentOrThread.id;
        if ('type' in commentOrThread && commentOrThread.type === 'thread') {
            ...
            <li
              ...
              <div className="CommentPlugin_CommentsPanel_List_Thread_QuoteBox">
                <blockquote className="CommentPlugin_CommentsPanel_List_Thread_Quote">
                  {'> '}
                  <span>{(commentOrThread as Thread).quote}</span>
                </blockquote>
              ...
              <ul className="CommentPlugin_CommentsPanel_List_Thread_Comments">
                {(commentOrThread as Thread).comments.map((comment: Comments) => (
                  <CommentsPanelListComment
                    key={comment.id}
                    comment={comment}
                    deleteComment={deleteCommentOrThread}
                    thread={commentOrThread as Thread}
                    rtf={rtf}
                  />
                ))}
              </ul>
        ...
        return (
          <CommentsPanelListComment
            key={id}
            comment={commentOrThread as Comment}
            deleteComment={deleteCommentOrThread}
            rtf={rtf}
          />
        );
...
export default function CommentPlugin({
  ...
  const commentsStoreInstance = useCommentStore();
  const comments = commentsStoreInstance;
  ...
  const deleteCommentOrThread = useCallback(
    (comment: Comment | Thread, thread?: Thread) => {
      if (!('type' in comment) || comment.type !== 'thread') {
        // For now, just delete the comment - a real implementation would handle threads properly
        commentStore.deleteCommentOrThread(comment.id);
        // TODO: Handle thread deletion properly
        // const deletionInfo = null;
        // if (!deletionInfo) {
        //   return;
        // }
  ...
  const submitAddComment = useCallback(
    (
      ...
      commentStore.addComment(commentOrThread);
      if (isInlineComment) {
        editor.update(() => {
          if ($isRangeSelection(selection)) {
  ...
  return (
    <>
      ...
      {showComments &&
        createPortal(
          <CommentsPanel
            comments={comments.getAllComments ? comments.getAllComments() : []}
            submitAddComment={submitAddComment}
            deleteCommentOrThread={deleteCommentOrThread}
            activeIDs={activeIDs}
            markNodeMap={markNodeMap}
